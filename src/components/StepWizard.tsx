...
export default function StepWizard({ onFormExpand }: StepWizardProps = {}) {
        ...
        return (
          <div className="space-y-6">
            ...
            {formData.serviceType !== 'post-construction' && formData.serviceType !== 'carpet' && formData.serviceType !== 'handyman' && (
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {ADDONS.filter((addon) => {
                  // Hide all addons for post-construction, carpet cleaning, and handyman
                  if (formData.serviceType === 'postconstruction' || formData.serviceType === 'carpet' || formData.serviceType === 'handyman') return false;
                  return true;
                }).map((addon) => {
                  const isAutoIncluded = 
                    (formData.serviceType === 'deep' && 
                  ...
                  return (
                  <label key={addon.key} className={`relative ${isAutoIncluded ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                    <input
                      type="checkbox"
                    ...
                    onMouseLeave={(e) => {
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className={`text-xs sm:text-sm font-medium ${isAutoIncluded || isSelected ? 'text-white' : 'text-white'}`}>{addon.label}</div>
                        <div className={`text-[10px] sm:text-xs ${isAutoIncluded || isSelected ? 'text-white/70' : 'text-white/70'}`}>
            ...
            <p className="text-[10px] sm:text-xs text-white/60 text-center leading-relaxed">
              By submitting this form, you agree to receive communications from Nooga Maids regarding your quote request.
              <br /><br />
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 2:
        return formData.firstName && formData.lastName && formData.email && formData.bedrooms && formData.bathrooms && formData.serviceType && formData.squareFootage && formData.zipCode;
      default:
        return false;
    }
  };
