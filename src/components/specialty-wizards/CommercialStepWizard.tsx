...
export default function CommercialStepWizard({ onFormExpand }: CommercialStepWizardProps) {
    ...
    try {
      ...
      const response = await fetch('https://formspree.io/f/mrbjzvde', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          _subject: `Tulsa Maids - Commercial Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
  ...
  if (currentStep === 4) return <SuccessMessage type="quote" confirmationNumber={formData.confirmationNumber} serviceLabel="commercial cleaning" inline={true} onClose={handleCloseSuccess} />;
