...
export default function CarpetStepWizard({ onFormExpand }: CarpetStepWizardProps) {
  ...
  const handleSubmit = async (retryCount = 0) => {
    ...
    try {
      const confirmationNumber = 'TM-CARPET-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      ...
      const response = await fetch('https://formspree.io/f/xvzwolek', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          _subject: `Tulsa Maids - Carpet Cleaning Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
  ...
  if (currentStep === 4) return <SuccessMessage type="quote" confirmationNumber={formData.confirmationNumber} serviceLabel="carpet cleaning" inline={true} onClose={handleCloseSuccess} />;
