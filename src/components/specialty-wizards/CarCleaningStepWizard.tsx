...
export default function CarCleaningStepWizard({ onFormExpand }: CarCleaningStepWizardProps) {
  ...
  const handleSubmit = async (retryCount = 0) => {
    ...
    try {
      const confirmationNumber = 'TM-CAR-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      const response = await fetch('https://formspree.io/f/mrbjzvde', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          _subject: `Tulsa Maids - Car Detailing Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
  ...
  if (currentStep === 4) return <SuccessMessage type="quote" confirmationNumber={formData.confirmationNumber} serviceLabel="car detailing" inline={true} onClose={handleCloseSuccess} />;
